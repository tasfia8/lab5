...
class TestFakeNewsAPI(unittest.TestCase):
    ...
    def test_real_news_1(self):
        response = requests.get('http://serve3-env.eba-pnvmffii.ca-central-1.elasticbeanstalk.com/predict?text=This is real news')
    ...
    def test_real_news_2(self):
        response = requests.get('http://serve3-env.eba-pnvmffii.ca-central-1.elasticbeanstalk.com/predict?text=Genuine report on environmental issues')
        data = json.loads(response.text)  # Parse the JSON response
        print(f"Test real_news_2 response: {data}")
        self.assertEqual(data['prediction'], 0)  # Expect 0 for REAL news

    def test_fake_news_1(self):
        response = requests.get('http://serve3-env.eba-pnvmffii.ca-central-1.elasticbeanstalk.com/predict?text=This is fake news')
        data = json.loads(response.text)  # Parse the JSON response
        print(f"Test fake_news_1 response: {data}")
        self.assertEqual(data['prediction'], 1)  # Expect 1 for FAKE news

    def test_fake_news_2(self):
        response = requests.get('http://serve3-env.eba-pnvmffii.ca-central-1.elasticbeanstalk.com/predict?text=Totally fabricated story about celebrities')
        data = json.loads(response.text)  # Parse the JSON response
